...
{
    public IResult<IList<CategoryOutDto>> Handle(int userId, bool onlyWithData)
    {
        return new Result<IList<CategoryOutDto>> { Data = mapper.Handle(repo.Index(userId, onlyWithData)) };
    }
}